// mock_auth_repo_test.go 提供 auth.Repository 接口的本地 mock 实现，供 workspace 测试使用。
//
// 引入动机：auth 包的 MockRepository 定义在 _test.go 文件中，无法跨包访问。
// workspace 测试需要 auth.Repository 的 mock 来构建完整 middleware 链，
// 因此在本地创建一个最小化的 mock 实现。
package workspace
...
// 引入动机：workspace 测试需要注入 auth.Repository 到 AuthMiddleware，
