...
// UpdateUserEmail 实现 Repository 接口。
...
// UpdateUserPasswordHash 实现 Repository 接口。
func (m *MockRepository) UpdateUserPasswordHash(ctx context.Context, userID, passwordHash string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	for _, user := range m.users {
		if user.ID == userID {
