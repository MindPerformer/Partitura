...
/** 调优作用域：global 使用系统默认配置，workspace 允许单工作区覆盖。 */
export type TuningScope = "global" | "workspace";

export type TuningParameterValue = number | boolean | string | null;
export type TuningParameters = Record<string, TuningParameterValue>;
...
export interface TuningHistoryEntry {
  ...
  action: string;
  parameters: TuningParameters | null;
  created_at: string;
  created_by: string;
}
...
export interface TuningConfig {
  ...
  base_profile: SearchProfile | Record<string, unknown>;
  effective_config: SearchProfile | Record<string, unknown>;
  draft: TuningParameters | null;
  published: TuningParameters | null;
  history: TuningHistoryEntry[];
  parameters: TuningParameterDefinition[];
  capabilities?: TuningCapabilities;
}
export interface TuningValidationCheck {
  code: string;
  passed: boolean;
  message: string;
}
...
export interface TuningValidationResponse extends TuningValidationResult {
  valid: boolean;
  checks: TuningValidationCheck[];
  effective_config: SearchProfile;
...
export interface ValidateTuningRequest {
  parameters: TuningParameters;
  queries?: string[];
  workspace_id?: string;
}
export interface PublishTuningRequest {
  expected_revision: number;
}
export interface RollbackTuningRequest {
  expected_revision: number;
...
export interface ProviderConfigDTO {
  ...
  api_key_set: boolean;
  config?:
    EmbeddingProviderConfig | RerankerProviderConfig | Record<string, unknown>;
  updated_at?: string;
}
