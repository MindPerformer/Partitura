...
const workspaceItems = computed<DropdownMenuItem[][]>(() => {
  ...
  if (items.length === 0) {
    items.push({
      label: t("workspace.noWorkspaces"),
      type: "label",
      disabled: true,
    });
  }
...
function performSearch() {
  ...
  router.push(
    `/workspaces/${props.workspaceId}/search?q=${encodeURIComponent(searchQuery.value.trim())}`,
  );
}
...
const userItems = computed<DropdownMenuItem[][]>(() => {
  ...
  if (isSystemAdmin.value) {
    ...
    actions.push({
      label: t("admin.adminWorkspaces"),
      icon: "i-lucide-folder",
      to: "/admin/workspaces",
    });
    actions.push({
      label: t("admin.tuning"),
      icon: "i-lucide-sliders-horizontal",
      to: "/admin/tuning",
    });
    actions.push({
      label: t("admin.searchProfiles"),
      icon: "i-lucide-settings",
      to: "/admin/search-profiles",
    });
    actions.push({
      label: t("admin.searchEvaluation"),
      icon: "i-lucide-bar-chart-3",
      to: "/admin/search-evaluation",
    });
    actions.push({
      label: t("admin.indexJobs"),
      icon: "i-lucide-cog",
      to: "/admin/jobs",
    });
    actions.push({
...
<template>
  <header class="sticky top-0 z-40 border-b border-default bg-default">
    <div class="flex h-14 items-center justify-between gap-2 px-2 sm:px-4">
      ...
      <div
        v-if="workspaceId"
        class="mx-2 hidden max-w-md flex-1 sm:block lg:mx-4"
      >
        <UInput
          v-model="searchQuery"
          ...
          <template #info="{ item }">
            <div class="py-1">
              ...
              <p class="text-xs text-muted">
                {{ systemRoleLabel(currentUser?.system_role) }}
              </p>
            </div>
          </template>
        </UDropdownMenu>
